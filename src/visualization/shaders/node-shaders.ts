/**
 * GLSL Shaders for Custom Node Material
 *
 * Ported from reference prototype's instanced node shaders.
 * Creates the distinctive swirling marble effect with Hilma af Klint mandala patterns.
 *
 * INV-A008: Use custom ShaderMaterial for direct fragment color control
 */
 ...
 * - Biography weight pulse animation
 * - Normal transformation for fragment shader
 */
...
/**
 * Instanced node fragment shader
 *
 * Implements:
 * - Fresnel rim glow
 * - Simplex noise for organic bioluminescence
 * - Hilma af Klint mandala patterns with sacred geometry
 * - Subsurface scattering simulation
 * - Bioluminescent spots
 * - Sacred colors (gold, rose, teal)
 */
...
// Hilma af Klint mandala with color symbolism
...
void main() {
  ...
  finalColor += baseColor * rimGlow * 0.8;  // Reduced from 1.5 to prevent wash-out
  ...
  if (uIsLightTheme > 0.5) {
    // The Seven Planetary Colors
    vec3 solarGold = vec3(0.85, 0.68, 0.22);
    vec3 lunarSilver = vec3(0.75, 0.78, 0.82);
    vec3 venusGreen = vec3(0.32, 0.52, 0.42);
    vec3 marsRed = vec3(0.72, 0.28, 0.22);
    vec3 jupiterBlue = vec3(0.22, 0.35, 0.58);
    vec3 saturnOchre = vec3(0.58, 0.48, 0.35);
    vec3 mercurySilver = vec3(0.68, 0.65, 0.72);
    ...
    // Dark theme: clamp colors to prevent wash-out
    finalColor = clamp(finalColor, vec3(0.0), vec3(1.0));
  }
