...
import { ModernRenderer } from './renderer';
import { DEFAULT_CONFIG } from './types';
import { generateFamilyWithNodeCount, generateLargeFamily } from './utils';
import type { Person, FamilyData } from './types';

// Use modern renderer with post-processing by default
type RendererType = ModernRenderer;
const AncestralWebRenderer = ModernRenderer;
...
class AncestralWebApp {
  ...
  private setupThemeToggle(): void {
    ...
    const savedTheme = localStorage.getItem('ancestral-web-theme');
    if (savedTheme === 'light') {
      document.documentElement.setAttribute('data-theme', 'light');
      this.updateRendererTheme('light');
    }
    ...
    toggle.addEventListener('click', () => {
      ...
      // Save preference
      localStorage.setItem('ancestral-web-theme', newTheme);

      // Update renderer background
      this.updateRendererTheme(newTheme);
    });
  }

  private updateRendererTheme(theme: 'light' | 'dark'): void {
    if (this.renderer) {
  ...
  async loadFamily(familyData: FamilyData): Promise<void> {
    ...
    try {
      ...
      this.renderer.onHover((person: Person | null, _screenPos: { x: number; y: number } | null) => {
        if (person) {
      ...
      this.renderer.onClick((person: Person | null) => {
...
export { AncestralWebApp, parseFamily, FamilyGraph, ForceDirectedLayout, ModernRenderer as AncestralWebRenderer };
