...
export class AncestralWebRenderer {
  ...
  private nodeUniforms: { uTime: THREE.IUniform; uColorPrimary: THREE.IUniform; uColorSecondary: THREE.IUniform; uGlowIntensity: THREE.IUniform; uIsLightTheme: THREE.IUniform } | null = null;
  ...
  constructor(container: HTMLElement, config: Partial<EngineConfig> = {}) {
    ...
    this.scene.fog = new THREE.FogExp2(0x0a0612, 0.0012);
   ...
   * Updates scene background, fog, lighting, and material blending for the selected theme
   */
  setTheme(theme: 'light' | 'dark'): void {
    ...
    if (isLight) {
      ...
      // Warm parchment background with rich jewel-tone elements
      const bgColor = 0xf5f0e6; // Warm parchment/vellum
      ...
      this.scene.traverse((object) => {
        if (object instanceof THREE.AmbientLight) {
          object.color.setHex(0xfff8f0); // Warm white
          object.intensity = 1.4;
        }
        if (object instanceof THREE.PointLight) {
          if (object.position.y > 0) {
            object.color.setHex(0xffe4b0); // Warm candlelight gold
            object.intensity = 1.2;
          } else {
            object.color.setHex(0xc9a227); // Lustrous gold accent
      ...
      const bgColor = 0x0a0612; // Deep cosmic indigo
      this.scene.background = new THREE.Color(bgColor);
    ...
    // Update node material - medieval manuscript medallions for light theme
    if (this.nodeInstancedMesh) {
      ...
      if (isLight) {
        // Deep lapis lazuli
        material.uniforms.uColorPrimary.value = new THREE.Color(0x19338c);
        // Warm gold leaf (less yellow)
        material.uniforms.uColorSecondary.value = new THREE.Color(0xd9a633);
        material.uniforms.uGlowIntensity.value = 0.2; // Minimal glow - painted look
      } else {
    ...
    // Update edge line materials - warm gold filigree for light theme
    for (const line of this.edgeLines) {
      const material = line.material as THREE.ShaderMaterial;
      material.blending = isLight ? THREE.NormalBlending : THREE.AdditiveBlending;
      material.uniforms.uIsLightTheme.value = isLight ? 1.0 : 0.0;
      if (isLight) {
        // Warm gold leaf (less yellow, more metallic)
        material.uniforms.uColorPrimary.value = new THREE.Color(0xd9a633);
        // Deep lapis accent
        material.uniforms.uColorSecondary.value = new THREE.Color(0x19338c);
    ...
    // Update shared event edges - warm gold leaf arcs
    for (const line of this.sharedEventEdges) {
      const material = line.material as THREE.ShaderMaterial;
      material.blending = isLight ? THREE.NormalBlending : THREE.AdditiveBlending;
      if (isLight) {
        material.uniforms.uColorGold.value = new THREE.Color(0xd9a633); // Warm gold
      } else {
    ...
    // Update particle systems - HIDE in light mode for clean parchment
    if (this.particleSystem) {
      const material = this.particleSystem.material as THREE.ShaderMaterial;
      material.blending = isLight ? THREE.NormalBlending : THREE.AdditiveBlending;
      if (isLight) {
        material.uniforms.uSize.value = 0; // Hide particles - clean parchment
      } else {
        material.uniforms.uSize.value = 10;
      }
      material.needsUpdate = true;
    }

    // Update background particles - HIDE in light mode
    if (this.backgroundParticles) {
      const material = this.backgroundParticles.material as THREE.ShaderMaterial;
      material.blending = isLight ? THREE.NormalBlending : THREE.AdditiveBlending;
      if (isLight) {
        material.uniforms.uSize.value = 0; // Hide - medieval manuscripts don't have floating dust
      } else {
        material.uniforms.uSize.value = 15;
      }
      material.needsUpdate = true;
    }

    // Update firefly system - bezants in light mode, sacred flames in dark mode
    if (this.fireflySystem) {
      const material = this.fireflySystem.material as THREE.ShaderMaterial;
      material.blending = isLight ? THREE.NormalBlending : THREE.AdditiveBlending;
      // Add uIsLightTheme uniform if it doesn't exist
      if (!material.uniforms.uIsLightTheme) {
        material.uniforms.uIsLightTheme = { value: 0.0 };
      }
      material.uniforms.uIsLightTheme.value = isLight ? 1.0 : 0.0;
      if (isLight) {
        material.uniforms.uSize.value = 6; // Small bezants like manuscript decoration
      } else {
        material.uniforms.uSize.value = 12;
      }
      material.needsUpdate = true;
    }

    // Update sacred geometry grid - vibrant manuscript colors (ultramarine/malachite)
    this.scene.traverse((object) => {
      if (object instanceof THREE.Line && object.material instanceof THREE.LineBasicMaterial) {
        ...
        if (isLight) {
          object.material.opacity = 0.4; // Prominent like zodiac wheel diagrams
          object.material.color.setHex(0x2c5a8c); // Deep ultramarine blue - like lapis lazuli ink
        } else {
  ...
  private createBackgroundParticles(): void {
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Esoteric palette: mix of violet (0.75), gold (0.12), and rose (0.95)
      const colorChoice = Math.random();
      let hue: number;
      if (colorChoice < 0.5) {
        hue = 0.75 + Math.random() * 0.08; // Violet range
      } else if (colorChoice < 0.8) {
        hue = 0.10 + Math.random() * 0.05; // Gold range
      } else {
        hue = 0.95 + Math.random() * 0.05; // Rose range
      }
  ...
  /**
   * Create sacred geometry mandala grid - concentric rings and radial lines
   * Inspired by Tibetan mandalas and Hilma af Klint's spiritual diagrams
   */
  private createSacredGeometryGrid(): void {
    const gridGroup = new THREE.Group();

    // Sacred gold color with transparency
    ...
    // Concentric circles (generation rings)
    const ringCount = 8;
    const ringSpacing = 50; // matches generationSpacing

    for (let i = 1; i <= ringCount; i++) {
      const radius = i * ringSpacing;
      const segments = 64 + i * 8; // More segments for outer rings
    ...
    // Radial lines (12 cardinal directions - like a clock or zodiac)
    ...
    // Position slightly below the nodes
